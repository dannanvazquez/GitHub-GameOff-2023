...
public class TerrainChecker
{
    ...
    private float[] GetTextureMix(Vector3 playerPos, Terrain t)
    {
        ...
        int mapX = Mathf.RoundToInt((playerPos.x - tPos.x) / tData.size.x * (tData.alphamapWidth - 1));
        int mapZ = Mathf.RoundToInt((playerPos.z - tPos.z) / tData.size.z * (tData.alphamapHeight - 1));
