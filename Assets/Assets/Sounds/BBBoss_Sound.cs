...
using UnityEngine;
using System;
public class bbboss_Sound : MonoBehaviour
{
    [Header("AudioClips")]
    [Tooltip("Sounds for bbboss melee attacking")]
    [SerializeField] private AudioClip[] bbboss_attack_voice;
    [SerializeField] private AudioClip[] bbboss_attack_sfx;
    [Tooltip("Sounds for bbboss dying")]
    [SerializeField] private AudioClip[] bbboss_death_voice;
    [SerializeField] private AudioClip[] bbboss_death_sfx;
    [Tooltip("Sounds for bbboss getting hit")]    
...
private AudioClip GetRandomClip(AudioClip[] clips, AudioClip lastClip)
{
    ...
    {
        PlayRandomClip(bbboss_attack_voice, ref lastAttackVoiceClip, AudioSource_Voice);
        PlayRandomClipNoLast(bbboss_attack_sfx, AudioSource_SFX);
    }

    public void PlayDeath()
    {
        PlayRandomClip(bbboss_death_voice, ref lastDeathVoiceClip, AudioSource_Voice);
        PlayRandomClip(bbboss_death_sfx, ref lastDeathSFXClip, AudioSource_SFX);
    }


    public void PlayHit()
    {
        PlayRandomClip(bbboss_hit_voice, ref lastHitVoiceClip, AudioSource_Voice);
        PlayRandomClip(bbboss_hit_sfx, ref lastHitSFXClip, AudioSource_SFX);
    }

        public void PlayThrow()
    {
        PlayRandomClipNoLast(bbboss_throw_voice, AudioSource_Voice);
        PlayRandomClipNoLast(bbboss_throw_sfx, AudioSource_SFX);
    }
...
void Update(){
    // Detect bbboss input for running
    isRunning = Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift);
        // Check if left footstep flag is set and play left footstep
        if (playLeftFootstep)
        {
            // Check if the bbboss is running and call the appropriate method
