    ...
    public class PlayerSound : MonoBehaviour
    {
        ...
        [Tooltip("Sounds for player getting hit")]    
        [SerializeField] private AudioClip[] player_roll_voice;
        [SerializeField] private AudioClip[] player_roll_sfx;
        ...
        [SerializeField] private GameObject dust_trail_jump;
        [SerializeField] private GameObject dust_trail_run;
        [SerializeField] private GameObject dust_trail_roll;
    ...
    IEnumerator FadeMuffledSound()
    {
        ...
        float transitionOutDuration = .25f; // Duration for fading out
        float transitionInDuration = 1.0f;  // Duration for fading back in

        // Fade out
        while (elapsedTime < transitionOutDuration)
        {
            float currentMuffledValue = Mathf.Lerp(not_muffled, muffled, elapsedTime / transitionOutDuration);
        ...
        // Wait for a short delay before fading back in
        ...
        {
            float currentMuffledValue = Mathf.Lerp(muffled, not_muffled, elapsedTime / transitionInDuration);
            myAudioMixer.SetFloat("MUFFLED_SOUND", currentMuffledValue);
            elapsedTime += Time.deltaTime;
            yield return null;
        }
        ...
        private void PlayRandomClipNoLast(AudioClip[] clips, AudioSource audioSource)
{
    ...
    {
        Debug.LogError("Empty audio clip array!");
    }
}
        ...
        public void PlayRoll(){
            dust_trail_roll.SetActive(true);
            PlayRandomClipNoLast(player_roll_voice, AudioSource_Voice);
            PlayRandomClipNoLast(player_roll_sfx, AudioSource_SFX);
        }
        public void PlayJump()
        {
            dust_trail_run.SetActive(false);
            dust_trail_roll.SetActive(false);
            ...
            if (dust_trail_jump != null)
            {
                dust_trail_jump.SetActive(true);

                // Deactivate the effect after a certain duration
                //Invoke("DeactivateEffect", 2f); // Adjust the duration as needed
            }
        }
    ...
    void PlayWalkingFootstep(bool isLeftFoot)
    {
        ...
        dust_trail_roll.SetActive(false);
        dust_trail_run.SetActive(false);
        dust_trail_jump.SetActive(false);
        AudioClip clip;
